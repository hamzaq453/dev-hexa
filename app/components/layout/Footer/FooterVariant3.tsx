  ...
  <footer className="w-full bg-background border-t border-primary-darkest text-foreground py-20 px-4 sm:px-8 lg:px-16">
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:justify-between md:items-start gap-10 md:gap-6">
      ...
      <div className="flex flex-col items-start md:w-1/5 mb-8 md:mb-10">
        ...
        <div className="text-xs text-foreground/60 tracking-widest font-medium">
          ...
          <div key={idx} className="flex flex-col gap-1">
            {col.links.map((link) => (
              <a
                key={link}
                href="#"
                className="uppercase text-xs font-semibold tracking-wider text-foreground/90 hover:text-primary transition-colors duration-200"
              >
                {link}
              </a>
            ))}
    ...
    <div className="flex justify-center gap-4 mb-6">
      {socialLinks.map((link, idx) => (
        <a
          key={link.name}
          href="#"
          className="w-10 h-10 rounded-full border border-foreground/30 flex items-center justify-center hover:bg-primary/10 hover:text-primary transition-colors duration-200"
        >
          {link.icon}
        </a>
      ))}
